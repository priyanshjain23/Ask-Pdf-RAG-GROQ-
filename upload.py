...
def process_txt(file) -> list[str]:
    text = file.read().decode("utf-8")
    text = re.sub(r"\s+", " ", text).strip()
    return split_text_to_chunks(text)


def process_json(file) -> list[str]:
    data = json.load(file)
    text = json.dumps(data, ensure_ascii=False)
    text = re.sub(r"\s+", " ", text).strip()
    return split_text_to_chunks(text)
...
def split_text_to_chunks(text, max_chunk_size=1000) -> list[str]:
    import re
...
def append_to_vault(chunks: list[str]):
