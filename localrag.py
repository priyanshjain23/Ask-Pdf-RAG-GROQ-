...
#         rewritten_query, vault_embeddings, vault_content
#     )
#     if relevant_context:
...
# parser.add_argument(
#     "--model", default="mistral-saba-24b", help="Ollama model to use (default: mistral-saba-24b)"
# )
...
# ANSI escape codes removed, not needed for Streamlit
...
def load_vault() -> list[str]:
    if os.path.exists("vault.txt"):
        with open("vault.txt", "r", encoding="utf-8") as vault_file:
            return [line.strip() for line in vault_file if line.strip()]
    return []


def get_vault_embeddings(vault_content: list[str]) -> torch.Tensor:
    embeddings = []
    for content in vault_content:
        response = ollama.embeddings(model="mxbai-embed-large", prompt=content)
...
def get_relevant_context(query, vault_embeddings, vault_content, top_k=5):
    if vault_embeddings.nelement() == 0:
        return []
    input_embedding = ollama.embeddings(model="mxbai-embed-large", prompt=query)[
        "embedding"
    ]
    cos_scores = torch.cosine_similarity(
        torch.tensor(input_embedding).unsqueeze(0), vault_embeddings
...
- Expand and clarify
- Avoid new topics
- DO NOT ANSWER the original query, only rewrite
...
def ollama_chat(
    ...
    # Create a separate user_input_with_context for the model only
    user_input_with_context = user_input
    if relevant_context:
        user_input_with_context += "\n\nRelevant Context:\n" + context_str
    DEBUG_CONTEXT = True
    # Print context to terminal for debugging (optional)
    if relevant_context and DEBUG_CONTEXT:
        print("\n🔍 Relevant Context Retrieved:")
        for i, ctx in enumerate(relevant_context, 1):
            print(f"\n--- Context {i} ---\n{ctx}")

    # Keep user input in conversation history clean for Streamlit UI
    conversation_history[-1]["content"] = user_input  # Only original user input

    # Inject the full context to the model without modifying UI-visible history
    messages = [
